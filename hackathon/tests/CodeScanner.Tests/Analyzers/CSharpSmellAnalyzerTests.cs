...
public class CSharpSmellAnalyzerTests
{
    ...
    public void ShortMethod_NoFindings()
    {
        ...
        Assert.Empty(Analyzer.Analyze("a.cs", src).Findings);
    }
    ...
    public void LongFunction_50Lines_NoFinding()
    {
        ...
        var src = $"class C {{\n    void Foo() {{\n{body}    }}\n}}";
        var findings = Analyzer.Analyze("a.cs", src).Findings.Where(f => f.Type == "long_function").ToList();
    ...
    public void LongParameterList_5Params_NoFinding()
    {
        ...
        Assert.DoesNotContain(Analyzer.Analyze("a.cs", src).Findings, f => f.Type == "long_parameter_list");
    }
    ...
    public void LongParameterList_6Params_LowSeverity()
    {
        ...
        var findings = Analyzer.Analyze("a.cs", src).Findings.Where(f => f.Type == "long_parameter_list").ToList();
        Assert.Single(findings);
        Assert.Equal("low", findings[0].Severity);
...
class C {
    void Foo() {
        ...
        Assert.DoesNotContain(Analyzer.Analyze("a.cs", src).Findings, f => f.Type == "deep_nesting");
    }
...
class C {
    void Foo() {
        ...
        var findings = Analyzer.Analyze("a.cs", src).Findings.Where(f => f.Type == "deep_nesting").ToList();
        Assert.Single(findings);
        Assert.Equal("low", findings[0].Severity);
    ...
    public void Lambdas_DoNotTriggerLongFunction()
    {
        ...
        var findings = Analyzer.Analyze("a.cs", sb.ToString()).Findings
    ...
    public void AutoGeneratedFile_IsSkipped()
    {
        ...
        var findings = Analyzer.Analyze("Generated.cs", src).Findings;
        Assert.Empty(findings);
    }
...
class C {
    ...
    void Lambdas() { System.Action a = () => {}; System.Action<int> b = x => {}; }
        ...
        // method A, ctor, dtor, operator +, conversion to string, Outer, Local, Lambdas = 8
        // The anonymous lambdas (`() => {}` and `x => {}`) should NOT count.
